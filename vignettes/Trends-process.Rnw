...
If \href{http://www.r-project.org/}{R} (version $\geq$ 3.1) is not already installed on your computer, download and install the latest binary distribution from the Comprehensive R Archive Network (\href{http://cran.r-project.org/}{CRAN}).
...
Support for merging Portable Document Format (PDF) files into a new file requires \href{http://www.pdflabs.com/tools/pdftk-server/}{PDFtk Server}, a cross-platform command-line tool for working with PDFs; download and install this software.
...
Run trend analysis for volotile organic compond (VOC) parameters from 1987 through 2012:
<<>>=
...
Run trend analysis for Carbon Tetrachloride (P32102), a VOC parameter, at the RWMC Production well (433002113021701) from 2005 through 2012:
<<>>=
...
Run trend analysis for Chloride (CL), a NWQL parameter, at the USGS 114 (433318112555001), PW 8 (433456112572001), USGS 87 (433013113024201), USGS 35 (433339112565801), and USGS 39 (433343112570001) wells from 1989 through 2012:
<<>>=
file <- file.path(path.in, "Config_NWQL.tsv")
config <- do.call(read.table, c(list(file), read.args))
processed.config <- ProcessConfig(config, processed.obs)
...
RunAnalysis(processed.obs, processed.config[rec, ], path = path.out,
            id = "Test_1989-2012",sdate = "1989-01-01", edate = "2012-12-31",
            graphics.type = graphics.type)
@

Account for water-level variability in the trend model.
<<>>=
RunAnalysis(processed.obs, processed.config[rec, ], path = path.out,
            id = "Test_WLs",sdate = "1989-01-01", edate = "2012-12-31",
...
<<>>=
RunAnalysis(processed.obs, processed.config[rec, ], path = path.out,
            id = "Test_Season",sdate = "1989-01-01", edate = "2012-12-31",
            graphics.type = graphics.type, is.seasonality = TRUE)
@

Account for water-level variability and seasonal variability in the trend model.
<<>>=
RunAnalysis(processed.obs, processed.config[rec, ], path = path.out,
            id = "Test_WLs_Season",sdate = "1989-01-01", edate = "2012-12-31",
            graphics.type = graphics.type, is.seasonality = TRUE,
            explanatory.var = processed.wl)
@
