...
Load the \href{https://github.com/jfisher-usgs/Trends}{Trends} package into the
current \textbf{R} session:
<<>>=
library(Trends)
@
...
Set the full path name to the directory containing input files; list all
...
To reprocess the 2014 water quality data, first view \textbf{R} code extracted
from this vignette. Manually transfer the code into an \textbf{R} session via a
