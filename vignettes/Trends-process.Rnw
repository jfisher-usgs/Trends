...
<<>>=
d <- ReadObservations(file.path(path.in, "Raw_Data.tsv"))
@

Read parameter configurations from a text file:
<<>>=
parameters <- ReadParConfig(file.path(path.in, "Parameters.tsv"))
@
...
PlotObservations(d, parameters = parameters, plot.config = plot.config,
...
out <- RunTrendAnalysis(d, parameters = parameters, plot.config = plot.config,
...
Run a trend analysis for censored VOC parameters from 1987 through 2012:
<<>>=
...
Run a trend analysis for uncensored VOC parameters from 1987 through 2012:
<<>>=
