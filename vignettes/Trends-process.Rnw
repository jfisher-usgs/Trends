...
try(knitr::opts_chunk$set(dev="pdf", tidy=FALSE), silent=TRUE)
options(warn=1)
options(width=90)
ptime <- proc.time()
@
...
If \href{http://www.r-project.org/}{R} is not already installed on your
computer, download and install the latest binary distribution from
\href{http://cran.r-project.org/}{CRAN}.
Open an \textbf{R} session and install required packages with the
following commands:
<<eval=FALSE>>=
...
Load the \textbf{Trends} package into the current \textbf{R} session:
<<>>=
library(Trends)
@

Support for merging PDF files into a new file requires
\href{http://www.pdflabs.com/tools/pdftk-server/}{PDFtk Server}, a
cross-platform command-line tool for working with PDFs; download and install
this software. Determine if \textbf{pdftk} is accessible from \textbf{R}:
<<>>=
print(merge.pdfs <- as.logical(nchar(Sys.which("pdftk"))))
@

Set the full path name to the directory containing input files; list the
files in this directory:
<<>>=
list.files(path.in <- system.file("extdata", "SIR2014", package = "Trends"))
@

Set the full path name to the output directory; if it doesn't already exist,
...
Read parameter options from a text file:
<<>>=
par.config <- ReadParConfig(file.path(path.in, "Config_Par.tsv"))
@

Read site locations from a shapefile:
<<>>=
...
View \textbf{R} code extracted from this vignette:
<<eval=FALSE>>=
browseURL(system.file("doc", "Trends-process.R", package = "Trends"))
@
Manually transfer this code into an \textbf{R} session via a copy and paste
operation to reprocess the data.

<<echo=FALSE>>=
ptime <- format(round((proc.time() - ptime)["elapsed"] / 60, digits=0))
@
Total processing time for this vignette was \Sexpr{ptime} minutes.
Version information about \textbf{R} and attached or loaded packages is as follows:
